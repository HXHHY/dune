...
namespace DUNE
{
  namespace IMC
  {
    ...
    bool
    TargetState::fieldsEqual(const Message& msg__) const
    {
      const IMC::TargetState& other__ = static_cast<const TargetState&>(msg__);
      if (lat != other__.lat) return false;
      if (lon != other__.lon) return false;
      if (x != other__.x) return false;
      if (y != other__.y) return false;
    ...
    int
    TargetState::validate(void) const
    {
      if (lat < -1.5707963267948966 || lat > 1.5707963267948966) return false;
      if (lon < -3.141592653589793 || lon > 3.141592653589793) return false;
      return true;
    ...
    uint8_t*
    TargetState::serializeFields(uint8_t* bfr__) const
    {
      uint8_t* ptr__ = bfr__;
      ptr__ += IMC::serialize(lat, ptr__);
      ptr__ += IMC::serialize(lon, ptr__);
    ...
    uint16_t
    TargetState::deserializeFields(const uint8_t* bfr__, uint16_t size__)
    {
      const uint8_t* start__ = bfr__;
      bfr__ += IMC::deserialize(lat, bfr__, size__);
      bfr__ += IMC::deserialize(lon, bfr__, size__);
    ...
    uint16_t
    TargetState::reverseDeserializeFields(const uint8_t* bfr__, uint16_t size__)
    {
      const uint8_t* start__ = bfr__;
      bfr__ += IMC::reverseDeserialize(lat, bfr__, size__);
      bfr__ += IMC::reverseDeserialize(lon, bfr__, size__);
    ...
    void
    TargetState::fieldsToJSON(std::ostream& os__, unsigned nindent__) const
    {
      IMC::toJSON(os__, "lat", lat, nindent__);
      IMC::toJSON(os__, "lon", lon, nindent__);
      IMC::toJSON(os__, "x", x, nindent__);
